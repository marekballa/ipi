...
if [ "$MODE" = "restart" ]; then
  ...
  # ADDED: bake CH nginx template into the image so no runtime nginx mount is needed
  # Written to build-context root (not into dtk-mfe-shell/ — that gets clone-populated by docker:init).
  # The awk-injected `RUN cp` above then overrides the docker:init-produced template with this one.
